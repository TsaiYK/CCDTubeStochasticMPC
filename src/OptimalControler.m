...
%             obj.x_max = max(Xc.V, [], 2)';
            ...
            if iscell(Xc)
                [C_ineq1, C_ineq2] = obj.construct_ineq_constraint_varying(Xc, Uc);
            else
        ...
        function [x_seq, u_seq, infeasible] = solve(obj)
            ...
            options = optimoptions('quadprog', 'Display', 'none');%, 'MaxIterations', 1000);
            [var_optim, ~, exitflag] = quadprog(obj.H, [], C_ineq1, C_ineq2, C_eq1, C_eq2, [], [], [], options);
%             any_infeasible_ineq = isempty(find(C_ineq1*var_optim-C_ineq2>0,1));
%             any_infeasible_eq = isempty(find(abs(C_eq1*var_optim-C_eq2)>1e-3,1));
            if exitflag==-2
                disp('exitflag = -2: the QP problem is infeasible!')
                infeasible = true;
                x_seq = []; u_seq = [];
%             elseif ~any_infeasible_ineq || ~any_infeasible_eq
%                 % This means some constraint violations from quadprog
%                 disp('exitflag = 0, but some constraints are violated!')
%                 infeasible = true;
%                 x_seq = []; u_seq = [];
            else
    ...
    methods (Access = private)
        ...
        function [C_ineq1, C_ineq2] = construct_ineq_constraint_varying(obj, Xc, Uc)
            % compute C_ineq (Now the state and control constraint sets are time-varying)
            num_poly = length(Xc);
            if num_poly ~= (length(Uc)+1)
                error('Number of Xc should have one more than the one of Uc!')
            end
            [F{1}, G{1}, nc] = convert_Poly2Mat(Xc{1}, Uc{1});
            F{1} = zeros(nc,obj.sys.nx);
            % G{1} = zeros(nc,obj.sys.nu); G{1}(obj.sys.nx*2+1:end) = 1;
            for i = 2:num_poly-1
                [F{i}, G{i}, nc] = convert_Poly2Mat(Xc{i}, Uc{i});
            end
            [F{num_poly}, ~, ~] = convert_Poly2Mat(Xc{num_poly}, Uc{num_poly-1});

            F_block = [];
            G_block = [];
            for itr = 1:obj.N
                G_block = blkdiag(G_block, G{itr});
            end
            for itr = 1:obj.N+1
                F_block = blkdiag(F_block, F{itr});
            end
            C_ineq1 = [F_block, [G_block; zeros(nc, obj.sys.nu*obj.N)]];
            nc_total = size(C_ineq1, 1);
            C_ineq2 = ones(nc_total, 1);
